...
back = pygame.image.load('mid_pic.jpg')
...
game_over_snd = pygame.mixer.Sound("game_over.ogg") 
...
bot_pic = pygame.image.load('arc_warden.png')
bot_list = []
lost = 0
...
class Bot(GameSprite):
    def __init__(self, x, y, w, h, image, speed):
        ...
        self.x_wait = randint(120,180)

    def update(self):
        global lost
        self.rect.y += self.speed
        if self.x_wait <= 0 :
            self.x_wait = randint(90,120)
            self.x_speed *= -1
...
bullet_pic = pygame.image.load('setka.png')

player_pic = pygame.image.load('meppo_pic.png')
player1 = Player(375, 420, 50, 60, player_pic, 4, 20)
...
while game:
    if screen == 'menu':
        ...
        new_lb =font.render ("Натисніть ENTER щоб почати гру", True, (255,255,255)) 
    ...
    if screen == 'game':
        ...
        score_lb = font.render('Score : ' + str(score), True, (255,255,255))
        bullets_lb =font.render ("Bullets:" + str(player1.have_bullets), True, (255,255,255)) 
        window.blit(bullets_lb,(0,80))
        window.blit(score_lb, (0,40))
        hp_lb = font.render('HP : ' + str(hp), True, (255,255,255))
        window.blit(hp_lb, (0,120))
        ...
        if hp == 0 or lost == 3:
            if score > max_score:
                with open('record.txt','w') as file:
                    ...
                    record = 1
            screen = 'gameover'
        elif pygame.sprite.spritecollide(player1, enemys_group, True, False):
            hp -=1 
    ...
    if screen == 'gameover':
        ...
        window.blit(new_lb,(105,125))
        score_lb = font.render('Знищенно: ' + str(score), True, (255,255,255))
        window.blit(score_lb, (250,200))
        if record == 1:
            record_lb = font.render('Новий рекорд!', True, (255,255,255))
            window.blit(record_lb, (250,275))
        else:
            record_lb = font.render('Рекорд: ' + str(max_score), True, (255,255,255))
            window.blit(record_lb, (250,275))
